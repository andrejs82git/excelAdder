...
public class Model {
  ...
  private UtfResourceBundle bundle;
  ...
  public Model() {
    ...
    this.pcs.addPropertyChangeListener(CHANGE_SRC_DIR, evt -> {
      ...
      if (templateList.length == 0) {
        ...
        Model.this.sendMsg(
            bundle.getString("Model.RuntimeException.MoreOneTemplateInDirectory.text") + ":" + templateList.toString());
        setTemplateFile(null);
      } else {
  ...
  public void setSrcDir(File newSrcDir) {
    if (!newSrcDir.isDirectory())
      throw new RuntimeException(bundle.getString("Model.RuntimeException.DirectoryNotChoosed.text"));
